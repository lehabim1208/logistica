...
export function ReviewView({ routeInfo, onStartDelivery, onCancel }: ReviewViewProps) {
  ...
  const saveEdit = (id: string, originalAddress: string, originalCoords: string, originalComments: string = '') => {
    if (editAddress.trim() === originalAddress.trim() && editCoords.trim() === originalCoords.trim() && editComments.trim() === originalComments.trim()) {
      setEditingId(null);
      setEditCoords('');
      setEditComments('');
      return;
    ...
    setOrders(o => o.map(order => order.id === id ? { ...order, address: editAddress, coords: editCoords, comments: editComments, isAmbiguous: false } : order));
    setEditingId(null);
    setEditCoords('');
    setEditComments('');
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 gap-4">
        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="text-sm text-gray-500 dark:text-gray-400 font-medium">Total de pedidos consolidados</div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {editingId === order.id ? (
                            <div className="flex flex-col gap-2 mt-3 bg-blue-50/50 p-3 rounded-lg border border-blue-100">
                              ...
                              <p className="text-xs text-gray-600 font-medium mt-1">Comentarios:</p>
                              <div className="flex items-center justify-between gap-2">
                                <input 
                                  type="text"
                                  value={editComments}
                                ...
                                <div className="flex items-center gap-1 shrink-0">
                                  <button onClick={() => { setEditingId(null); setEditCoords(''); setEditComments(''); }} className="text-gray-600 dark:text-gray-400 bg-gray-200 dark:bg-gray-700 p-2 hover:bg-gray-300 dark:hover:bg-gray-600 rounded-lg transition-colors" title="Cancelar">
                                    <X className="w-5 h-5" />
                                  </button>
                                  <button onClick={() => saveEdit(order.id, order.address, order.coords || '', order.comments || '')} className="text-white bg-blue-600 p-2 hover:bg-blue-700 rounded-lg shadow-sm transition-colors" title="Guardar">
                            ...
                            <div className="flex items-start gap-2 mt-1">
                              ...
                              <div className="flex flex-col flex-1 min-w-0">
                                ...
                                {order.coords && (
                                  <span className="block text-xs text-blue-600 dark:text-blue-400 font-mono mt-1 truncate">
      ...
      <div className="flex gap-4">
        ...
        >
          ...
          {orders.some(o => o.delivered) ? 'Continuar Reparto' : 'Iniciar Reparto'}
        </button>
      </div>
    </div>
  );
